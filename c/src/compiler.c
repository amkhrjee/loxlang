...
{
    // suppress errors if panic mode is already triggered
    if (parser.panicMode)
        return;
...
static uint8_t makeConstant(Value value)
{
    ...
    // this is because the operand of OP_CONSTANT
...
static void function(FunctionType type)
{
    ...
    consume(TOKEN_LEFT_PAREN, "Expect ')' after function parameters.");
...
static void ifStatement()
{
    ...
    emitByte(OP_POP); // pops the condition from the stack
    statement();

    int elseJump = emitJump(OP_JUMP);

    patchJump(thenJump);
    emitByte(OP_POP); // in case the condition is falsey
    if (match(TOKEN_ELSE))
        statement();
